...
def check_scalar_ref(scalar_ref):
    def myassert(ref):
        if not ref.default_value:
            raise Exception("{}: {}.{} needs to have a default value".format(get_model(ref)._tx_filename,ref.parent.name,ref.name))
    if scalar_ref.ref2:
...
def check_array_dimensions(array_dimension):
    if len(array_dimension.parent.array_dimensions)>1:
        if not array_dimension.array_index_name:
            raise Exception("{}: array {} needs to have named dimensions: specify [SIZE:NAME]".format(get_model(array_dimension)._tx_filename,array_dimension.parent.name))
...
def check_array_attribute(array_attribute):
    ...
    for d in dependencies:
        if not(d in available_infos_until_this_array):
            raise Exception("array {}.{} depends on {}.{} not defined before it in {}.".format(struct.name, array_attribute.name, struct.name, d.name, get_model(struct)._tx_filename))
...
def check_raw_type_has_defined_bits(rawtype):
    if rawtype.genericType!="custom" and not rawtype.genericBits:
        raise Exception("Bits need to be specified for {} in {}".format(rawtype.name,
                                                                            get_model(
                                                                                rawtype)._tx_filename))
