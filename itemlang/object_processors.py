...
def check_array_dimensions(array_dimension):
    if len(array_dimension.parent.array_dimensions) > 1:
        if not array_dimension.array_index_name:
            raise Exception("{}: array {} needs to have named " +
                            "dimensions: specify [SIZE:NAME]".format(
                                get_model(array_dimension)._tx_filename,
                                array_dimension.parent.name))
...
def check_array_attribute(array_attribute):
    ...
    dependencies = map(
        lambda x: x.ref0, reduce(
            lambda l1, l2: l1 + l2, map(
                lambda node: get_children_of_type(ScalarRef, node),
    ...
    for d in dependencies:
        if not (d in available_infos_until_this_array):
            raise Exception("array {}.{} depends on {}.{} not defined " +
                            "before it in {}.".format(
        ...
        def check_raw_type_is_defined_for_language(rawtype):
            if "generate_cpp" in self.options.keys() \
                ...
                if rawtype.genericType == "custom" and not rawtype.cpptype:
                    raise Exception("C++ type is required to generate " +
                                    "C++ code for {} in {}".format(
                                        rawtype.name,
            ...
            if "generate_python" in self.options.keys() \
                ...
                if rawtype.genericType == "custom" and not rawtype.pythontype:
                    raise Exception("python type is required to generate " +
                                    "python code for {} in {}".format(
                                        rawtype.name,
                                        get_model(rawtype)._tx_filename))

        for t in set(filter(lambda x: isinstance(x, RawType),
                            map(lambda x: x.type, struct.attributes))):
